...
You'll create a complete **three-tier database system** from scratch: an interactive **client**, a networked **server**, and a persistent **database engine**. This mirrors real-world software architecture and teaches systems programming fundamentals.
...
- Multiple input methods (interactive, file, pipe, arguments)
...
│             │                │             │◄──────────────►│   Engine    │
│ - CLI       │                │ - TCP       │                │ - Parser    │
│ - Formatter │                │ - Protocol  │                │ - Storage   │
│ - Network   │                │ - Logging   │                │ - Indexes   │
└─────────────┘                └─────────────┘                └─────────────┘
```

### Data Flow Example
1. **User Input**: `SELECT name, age FROM users WHERE age > 25;`
2. **Client**: Parses command, sends JSON request to server
3. **Server**: Receives JSON, forwards SQL to database engine
4. **Database**: Lexes → Parses → Executes → Returns results
5. **Server**: Formats response as JSON, sends to client
